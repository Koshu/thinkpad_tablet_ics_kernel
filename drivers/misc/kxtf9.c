...
int kxtf9_set_pc1_off(void)
{
	u8 reg_data;

	reg_data = tf9->resume[RES_CTRL_REG1] & ~PC1_ON;

	return kxtf9_i2c_write(CTRL_REG1, &reg_data, 1);
}

int kxtf9_set_pc1_on(void)
{
	u8 reg_data;
...
static DEVICE_ATTR(delay, 0660, kxtf9_delay_show, kxtf9_delay_store);
static DEVICE_ATTR(enable, 0660, kxtf9_enable_show,kxtf9_enable_store);
static DEVICE_ATTR(tilt, 0660, kxtf9_tilt_show, kxtf9_tilt_store);
static DEVICE_ATTR(wake, 0660, kxtf9_wake_show, kxtf9_wake_store);
static DEVICE_ATTR(tap, 0660, kxtf9_tap_show, kxtf9_tap_store);
static DEVICE_ATTR(selftest, 0660, NULL, kxtf9_selftest_store);
...
static int kxtf9_ioctl(struct file *file, unsigned int cmd,
	   unsigned long arg)
{
	...
	switch (cmd) {
		...
		case KXTF9_IOCTL_ENABLE_OUTPUT:
			retval = kxtf9_set_pc1_on();
			if(retval < 0)
				goto err_out;
			break;

		case KXTF9_IOCTL_DISABLE_OUTPUT:
			retval = kxtf9_set_pc1_off();
			if(retval < 0)
				goto err_out;
			break;
