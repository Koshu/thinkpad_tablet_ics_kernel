...
config_callback		read_config_dispatcher	= NULL;
config_callback		write_config_dispatcher	= NULL;
message_callback	read_config_sensor	= NULL;
...
#define VIRTUAL_KEYS_FILE_PREFIX	"virtualkeys."	/* This is the constant part of the file name, will be concatenate with user defined part */

/** 
 * sysfs Local API
 */
static ssize_t ntrig_virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);

/* attributes buffer size - make sure it doens't exceed PAGE_SIZE (On i386, this is 4096 On i386) 
   Buffer holding the virutal keys definitions 
   (6 values:reserved:scan-code:x:y:w:h; 5 delimiters + \n" +final \0) ?* number of keys */
static char virtual_keys_attr_buf[(CHARS_PER_NUMBER*6+sizeof(char)*7)*VIRTUAL_KEYS_NUM];
static char virtual_keys_file_name[sizeof(VIRTUAL_KEYS_FILE_PREFIX)+sizeof(NTRIG_DEVICE_NAME)+1];

/* These structures support single virtual keys device.
   If more device should be supported in the futures, more static kobj_attribute can be added,
   ntrig_properties_attrs should be replaced by an array and corresponding code should be change accordingle. */  
 ...
 * get bus interface
 */
static ssize_t get_bus_interface_show(struct kobject *kobj, struct kobj_attribute *attr,
			char *buf)
{
	return read_get_bus_interface((void*)buf);
}

static ssize_t get_bus_interface_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *buf, size_t count)
{
...
static struct kobj_attribute tracklib_status_attribute =
	...
	__ATTR(tracklib_status, 0660, config_dispatcher_show, config_dispatcher_store);
static struct kobj_attribute pen_status_attribute =
	...
	__ATTR(pen_status, 0660, config_dispatcher_show, config_dispatcher_store);
...
static struct kobj_attribute debug_print_attribute =
   ...
   __ATTR(debug_print, 0660, config_dispatcher_show, config_dispatcher_store);
 ...
 */
static ssize_t config_sensor_show(struct kobject *kobj, struct kobj_attribute *attr,
			char *buf)
{
...
static struct kobj_attribute config_sensor_attribute =
	...
	__ATTR(config_sensor, 0660, config_sensor_show, config_sensor_store);
...
static struct kobj_attribute get_bus_interface_attribute =
	...
	__ATTR(get_bus_interface, 0660, get_bus_interface_show, get_bus_interface_store);
