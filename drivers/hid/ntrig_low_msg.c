 ...
 * Utilities for building and parsing of low level messages received and sent to the sensor
 * over a low-level transport (SPI or i2c)
 * Sending messages:
 ...
 */
void set_low_msg_sm_data_packet(struct _ntrig_low_sm_info* info, u8* data, u16 length)
{
	info->data = data;
	info->dataLen = length;
...
void build_low_bus_msg_short_cmd(u8 cmd, struct _ntrig_low_bus_msg* msg)
{
	...
	/* detect if it is a FEATURE command - need a specific function for it */
	/* TODO use a lookup table to make it faster */
	if((cmd >= LOWMSG_FUNCTION_GET_MODE && cmd <= LOWMSG_FUNCTION_SET_MODE_DUAL) ||
	...
	/* for now: START_CALIBRATION, GET_CALIBRATION_RESULT len=0x04, GET_FW_VERSION len = 0x08 */
