...
struct ntrig_spi_privdata {
	...
	/** wait queue for waiting for command result after	sending
	 *  command */
...
struct watchdog_timer_t
{
	...
	bool is_enabled;	// when true - watchdog is enabled (configurable by /sys/spi_watchdog/watchdog)
	bool is_suspended;	// true when driver is suspended
	bool is_exit;		// set this to true to exit the watchdog timer
	bool is_msg_sent;	// true if watchdog msg sent to the device
	int msg_count;		// number of consecutive watchdog msgs sent to the device
...
static int spi_write_hid_ncp (void *dev, uint8_t cmd, const char *buf, short msg_len);
...
{	
	if (watchdog_timer.is_enabled && !watchdog_timer.is_suspended)
	{
		...
		{
			// not enough communication with the sensor. Send a message to verify communication.
...
static int setup_transfer(struct ntrig_spi_privdata *data)
{
	...
	/* make sure you fill the length correctly before doing an SPI transfer, up to MAX_SPI_TRANSFER_BUFFER_SIZE */
 ...
 */
static ssize_t ntrig_spi_test_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count);
static ssize_t ntrig_spi_test_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
...
static void spi_process_message(struct ntrig_spi_privdata* privdata)
{
	...
	case LOWMSG_CHANNEL_SINGLETOUCH:
		if(msg->function == LOWMSG_FUNCTION_ST_REPORT) {
			/* single touch report is actually sent to multi-touch channel. We feed it to tracklib
			   as it involves fingers (the upper level may optimize and bypass tracklib) */
			struct _ntrig_low_st_report* str = (struct _ntrig_low_st_report*)&msg->data[0];
			uint8_t valid1st = str->flags & 0x01; /* use in_range bit for "touch confidence" */
			ntrig_dbg("%s: ST flags = %u\n", __FUNCTION__, str->flags);
			mr->type = MSG_FINGER_PARSE;
			mr->msg.fingers_event.sensor_id = privdata->sensor_id;
			mr->msg.fingers_event.frame_index = privdata->stReportCount++; 
	...
	case LOWMSG_CHANNEL_CONTROL_REPLY:
		...
		/* copy the payload (after function) to the hid_ncp_reply_buf and wake up anyone waiting
		for reply (usually spi_write_ncp_hid). Note that we assume we are inside the spi_lock */
		//printk(KERN_ERR "********** Reply recieved. ID = 0x%x ************\n", msg->data[0]);
		spin_lock(&watchdog_timer.lock);
		if (watchdog_timer.is_msg_sent && msg->data[0] == LOWMSG_FUNCTION_GET_FW_VERSION)
		{
			//printk(KERN_ERR "********** Get FW Version reply recieved. Watchdog deactivated. ************\n");
		...
		privdata->hid_ncp_reply_size = msg->length - offsetof(struct _ntrig_low_msg, data);
		ntrig_dbg("%s: got control reply, size=%d\n", __FUNCTION__, privdata->hid_ncp_reply_size);
		if(privdata->hid_ncp_reply_size < 0 || privdata->hid_ncp_reply_size > MAX_HID_NCP_REPLY_SIZE) {
 ...
 * !!!MUST be called with spi_lock held!!! 
 */
static int execute_spi_bus_transfer(struct ntrig_spi_privdata* privdata, int len)
{
	int res, err;
	...
	if(err) {
		ntrig_dbg("%s: spi_sync failure, bailing out\n", __FUNCTION__);
		return err;
	}
	set_low_msg_sm_data_packet(&privdata->sm, privdata->rx_buf, privdata->xfer.len);
	do {
...
static irqreturn_t spi_irq(int irq, void *dev_id)
{
	...
	if(!spi_get_output_enabled(privdata)) {
		/* output_enable is low, meaning the sensor will not be able to access the SPI bus,
		   no point in trying any SPI transfer, so end here to avoid extra noise on the SPI bus.
...
static int spi_write_hid_ncp (void *dev, uint8_t cmd, const char *buf, short msg_len) 
{
	...
	struct ntrig_spi_privdata* privdata = (struct ntrig_spi_privdata*)spi_get_drvdata(spi);
	int err;
	...
	/* wait for reply. This is done without a lock since we can only have a single command/reply at any given time and we
	   don't accumulate the reply results */
	err = wait_event_interruptible_timeout(privdata->hid_ncp_wait_queue, (privdata->hid_ncp_got_result != 0), 
...
static int spi_write_raw (void *dev, const char *buf, short msg_len) 
{
	...
	case LOWMSG_REQUEST_NCP_DFU:
	{
		...
		if(request == LOWMSG_REQUEST_GO_TO_BOOTLOADER) {
			/* when executing go to bootloader, sensor will be reset. We must keep the output_enable low
 ...
 */
static int spi_get_raw_data_from_fifo(struct kfifo* fifo, char* buf, size_t count)
{
	u16 data_len;
...
{
	struct ntrig_spi_privdata* privdata = (struct ntrig_spi_privdata*)spi_get_drvdata(tmp_spi_dev);
	...
	/* register device in the dispatcher. We register twice - once for HID and once for RAW (ncp/bulk)
	   TODO we use a hard-coded bus name of "spi", need to change if we want to support multiple sensors connected over SPI */
	ncp_func.dev = spi;
...
static ssize_t ntrig_spi_watchdog_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count);
static ssize_t ntrig_spi_watchdog_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
...
static struct kobj_attribute ntrig_spi_watchdog_attr =
	...
	__ATTR(NULL, 0660, ntrig_spi_watchdog_show, ntrig_spi_watchdog_store);
...
static ssize_t ntrig_spi_watchdog_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *buf, size_t count)
{	
	/* data is not written into actual file on file system, but rather saves it in a memory buffer */
	ntrig_dbg( "inside %s \n", __FUNCTION__ );
...
static void ntrig_spi_watchdog_release(struct kobject *kobj)
{
	...
	/* kobj is pointer to properties_kobj which was allocated in ntrig_create_virtualkeys_file */
/*	kfree(kobj);  this might be redundant if we call kobject_put(kobj), and cause a crahs later */
...
static struct kobject* ntrig_create_spi_watchdog_file(void)
{
	...
	/* 0. Preparations - to deal with case of multiple calls to this function, with same ntrig dev name */
	...
	ntrig_dbg( "inside %s creating new attr_name: %s\n", __FUNCTION__, ntrig_spi_watchdog_attr.attr.name);	


	/* 1. Create folder "spi_watchdog" under "sys" */
	/* allocate the memory for the whole object */
	properties_kobj = kobject_create_and_add(NTRIG_SPI_WATCHDOG_DIRECTORY, NULL);
	ntrig_dbg( "inside %s\n", __FUNCTION__);	
...
static struct kobj_attribute ntrig_spi_test_attr =
	...
	__ATTR(NULL, 0660, ntrig_spi_test_show, ntrig_spi_test_store);
...
static void debug_print_msg(struct _ntrig_low_bus_msg* msg)
{
	...
	for(i=0; i<17; i++) {
		ntrig_dbg("msg part %d: %d %d %d %d %d %d %d %d\n", i,
				  buf[offset], buf[offset+1], buf[offset+2], buf[offset+3], buf[offset+4], buf[offset+5], buf[offset+6], buf[offset+7]);
		offset += 8;
	}
}
 ...
 */
static ssize_t ntrig_spi_test_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *buf, size_t count)
{
	struct ntrig_spi_privdata* privdata = (struct ntrig_spi_privdata*)spi_get_drvdata(tmp_spi_dev);
	char ch;
	...
	if(ch == '0') {
		...
		ntrig_dbg("%s: gpio_irq value is %d\n", __FUNCTION__, val);
			...
			/** test: turn on or off output_enable (1=turn off 2=turn on)
			 *  note, if an	inverter is	connected, the actual result will
			 *  be the opposite */
			int gpio_index;
			int val = ch - '1';
		...
		struct _ntrig_low_bus_msg* msg = (struct _ntrig_low_bus_msg*)(privdata->tx_buf);
		struct _ntrig_low_bus_msg* rmsg = (struct _ntrig_low_bus_msg*)(privdata->rx_buf);
		int err;
		int i;
		...
		/* PROBE: activate the driver, register to dispatcher. This emulates the part that should be executed
		...
		err = spi_sync(tmp_spi_dev, &privdata->msg);
		/* clear the tx_buf so we don't send unexpected commands to the sensor */
		...
		struct _ntrig_low_bus_msg* rmsg = (struct _ntrig_low_bus_msg*)(privdata->rx_buf);
		ntrig_dbg("%s: state=%d substate=%d\n", __FUNCTION__, privdata->sm.state, privdata->sm.substate);
		/* last received data */
		debug_print_msg(rmsg);
	} else if(ch == 'a') {
...
/*	kfree(kobj);  this might be redundant if we call kobject_put(kobj), and cause a crahs later */
	ntrig_dbg( "inside %s After call to kfree(kobj)\n", __FUNCTION__);	
#if 0
	/* ntrig_virtual_keys_attr.attr.name was allocated in ntrig_create_virtualkeys_file */
	if (tmpBuf != NULL) {
		kfree(tmpBuf);
...
static struct kobject* ntrig_create_spi_test_file(void)
{
	...
	/* 0. Preparations - to deal with case of multiple calls to this function, with same ntrig dev name */
	/* generate the file (attribute) name, from user data */
	len = strlen(NTRIG_SPI_TEST_FILE_NAME);
	attr_name =  kmalloc((len), GFP_KERNEL);
	sprintf(attr_name, NTRIG_SPI_TEST_FILE_NAME);
	ntrig_spi_test_attr.attr.name = attr_name;
	ntrig_dbg( "inside %s creating new attr_name: %s\n", __FUNCTION__, ntrig_spi_test_attr.attr.name);	


	/* 1. Create folder "spi_test" under "sys" */
	/* allocate the memory for the whole object */
	properties_kobj = kobject_create_and_add(NTRIG_SPI_TEST_DIRECTORY, NULL);
	ntrig_dbg( "inside %s\n", __FUNCTION__);	
...
static void ntrig_spi_early_suspend(struct early_suspend *handler)
{
	...
	printk("Ntrig Touch Screen: early suspend\n");
}

static void ntrig_spi_late_resume(struct early_suspend *handler)
{
...
static int __devinit ntrig_spi_probe(struct spi_device *spi)
{
	...
	/* init variables relatedto raw read/write
	   we use the spi_lock to protect the fifo, no additional lock is needed */
...
//cloud-0426start
//enable touch
	err = gpio_request(NTRIG_TOUCH_ENABLE, "ntrig_enable");
	err = gpio_direction_output(NTRIG_TOUCH_ENABLE, 1);
	msleep(500);
//cloud-0426end
...
//cloud-0602start
//for suspend/resume
	ntrig_spi_tsp = kzalloc(sizeof(struct ntrig_spi_privdata), GFP_KERNEL);
	ntrig_spi_tsp->spi = spi;

	//pdata->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
       pdata->early_suspend.suspend = ntrig_spi_early_suspend;
       pdata->early_suspend.resume = ntrig_spi_late_resume;
...
static int ntrig_spi_remove(struct spi_device *spi)
{
	...
	if (watchdog_timer.work_q)
	{
		watchdog_timer.is_exit = true;		// signal the task function to stop queueing new tasks
		cancel_delayed_work(&watchdog_task);	// cancel existing queued work
		flush_workqueue(watchdog_timer.work_q);	// wait for ongoing work to finish
	...
	UnregNtrigDispatcher(pdata->ntrig_dispatcher, pdata->sensor_id, TYPE_BUS_SPI_HID, "spi");	
	UnregNtrigDispatcher(pdata->ntrig_dispatcher, pdata->sensor_id, TYPE_BUS_SPI_RAW, "spi");	
