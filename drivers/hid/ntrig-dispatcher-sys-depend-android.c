...
void config_multi_touch(lp_ntrig_bus_device dev, struct input_dev*	input_device)
{
	...
	if (virtual_keys_supported()) {
		...
		{
			__set_bit(get_virt_keys_scan_code(i), input_device->keybit);
		} 	

		input_set_abs_params(input_device, ABS_MT_POSITION_X, dev->logical_min_x+get_touch_screen_border_left(), 
					dev->logical_max_x-get_touch_screen_border_right(),	0, 0);
		input_set_abs_params(input_device, ABS_MT_POSITION_Y, dev->logical_min_y+get_touch_screen_border_down(), 
		...
		input_set_abs_params(input_device, ABS_MT_POSITION_X	, dev->logical_min_x, 	dev->logical_max_x, 	0, 0);
	...
	input_set_abs_params(input_device, ABS_MT_TRACKING_ID, 0, ABS_MT_TRACKING_ID_MAX, 0, 0);
