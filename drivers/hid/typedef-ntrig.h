...
} GET_BUS_INTERFACE_REPORT;

#define FIRST_DEV_TYPE	TYPE_BUS_USB_HID
#define LAST_DEV_TYPE	TYPE_BUS_I2C_RAW

/* note: do not use 0bXXX for binary constants - not supported in gcc 4.2.1 which is used by x86 ndk */
...
//0x19 - unused
      ...
      REPORTID_GET_PEN_BATTRY_STATUS = 0xF0	// Dummy ID - status is read from SW, not device
} CONFIGURATION_TYPE_HEX;
 ...
 * Capacitive button
 */
typedef struct capacitive_button_s
{
	uint16_t 		btn_code; // scan code
	button_mode_t	btn_mode;
} capacitive_button_t;

typedef struct button_parse_s {
      uint8_t sensor_id;
      uint8_t num_of_buttons;
      capacitive_button_t buttons_array[MAX_NUMBER_OF_FINGER];
} button_parse_t;
 ...
 * track_id - finger_id
 */
typedef struct device_finger_s {
      uint8_t   track_id;
      uint16_t  x_coord;
      uint16_t  y_coord;
      uint16_t  dx;
 ...
 * finger_parse_s
 */
typedef struct finger_parse_s {
      uint8_t sensor_id;
      uint8_t num_of_fingers;
      uint16_t	frame_index;
      device_finger_t finger_array[MAX_NUMBER_OF_FINGER];
} finger_parse_t;

/**
 * mr_message_types_s
 */
typedef struct mr_message_types_s {
      MSG_TYPE		        type;
      union {
	  device_pen_t		pen_event;
	  finger_parse_t	fingers_event;
	  button_parse_t	buttons_event;
      } msg;
} mr_message_types_t, *lp_mr_message_types_t;


/*
 * N-trig Component Version
 */
typedef struct comp_ver_s {
 ...
 * N-trig NCP Message
 */
typedef struct ntrig_usbhid_ncp_s {
 ...
 * config_bit_mask - Bit 0 - Pen Bit 1 -TrackLib Bit 2 - bus_interface Bit 3 -Virtual Keys
 */
typedef struct dispatcher_config_s {
    uint8_t config_bit_mask;
    uint8_t pen_enable;
    uint8_t tracklib_enable;
    uint8_t bus_interface;
    char    *virtual_buttons;	// max size for this buffer is PAGE_SIZE (==4096)
